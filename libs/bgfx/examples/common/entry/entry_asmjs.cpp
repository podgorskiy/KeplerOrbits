...
namespace entry
{
	...
	struct Context
	{
		...
		int32_t run(int _argc, char** _argv)
		{
			...
			emscripten_set_touchstart_callback("#canvas", this, true, touchCb);
			emscripten_set_touchend_callback("#canvas", this, true, touchCb);
			emscripten_set_touchmove_callback("#canvas", this, true, touchCb);
			emscripten_set_touchcancel_callback("#canvas", this, true, touchCb);
	...
	extern  "C"
	{
		...
		{
			s_ctx.m_eventQueue.postSizeEvent(s_defaultWindow, w, h);
		}
	}
	...
	EM_BOOL Context::touchCb(int eventType, const EmscriptenTouchEvent* event, void* userData)
	{
		BX_UNUSED(userData);
		...
		if (event->numTouches==1)
		{
			...
			if(lastTouchCount>1 || id != tp->identifier)
			{
				s_ctx.m_eventQueue.postMouseEvent(s_defaultWindow, s_ctx.m_mx, s_ctx.m_my, s_ctx.m_scroll, MouseButton::Left, false);
				s_ctx.m_mx = tp->canvasX;
				s_ctx.m_my = tp->canvasY;
				s_ctx.m_eventQueue.postMouseEvent(s_defaultWindow, s_ctx.m_mx, s_ctx.m_my, s_ctx.m_scroll);
				id = tp->identifier;
			}
			lastTouchCount = event->numTouches;
			...
			switch (eventType)
			{
				...
				case EMSCRIPTEN_EVENT_TOUCHCANCEL:
				{
					s_ctx.m_mx = tp->canvasX;
					s_ctx.m_my = tp->canvasY;
					s_ctx.m_eventQueue.postMouseEvent(s_defaultWindow, s_ctx.m_mx, s_ctx.m_my, s_ctx.m_scroll, MouseButton::Left, (eventType == EMSCRIPTEN_EVENT_TOUCHSTART));
					return true;
				}
			}
		}
		...
		{
			const EmscriptenTouchPoint* tpA = &event->touches[0];
			const EmscriptenTouchPoint* tpB = &event->touches[1];
			long xAvrg = (tpA->canvasX + tpB->canvasX)/2;
			long yAvrg = (tpA->canvasY + tpB->canvasY)/2;
			
			static float lastLength = 0;
			float length = pow(pow((tpA->canvasX - tpB->canvasX), 2.0f) + pow((tpA->canvasY - tpB->canvasY), 2.0f), 0.5f);
			...
			if(lastTouchCount!=2)
			{
				...
				s_ctx.m_my = yAvrg;
				s_ctx.m_eventQueue.postMouseEvent(s_defaultWindow, s_ctx.m_mx, s_ctx.m_my, s_ctx.m_scroll);
				//s_ctx.m_eventQueue.postMouseEvent(s_defaultWindow, s_ctx.m_mx, s_ctx.m_my, s_ctx.m_scroll, MouseButton::Left, true);
				lastLength = length;
				lastTouchCount = event->numTouches;
				return true;
			}
			...
			switch (eventType)
			{
				case EMSCRIPTEN_EVENT_TOUCHMOVE:
				{
					s_ctx.m_mx = xAvrg;
					s_ctx.m_my = yAvrg;
					s_ctx.m_scrollf += (length - lastLength)*0.05f *100.0f;
	...
	EM_BOOL Context::wheelCb(int eventType, const EmscriptenWheelEvent* event, void* userData)
	{
		...
		if (event)
		{
			switch (eventType)
			{
				...
				{
					s_ctx.m_scrollf += event->deltaY *100.0f;
					printf("%f\n", event->deltaY);
					s_ctx.m_scroll = (int32_t)s_ctx.m_scrollf;
					s_ctx.m_eventQueue.postMouseEvent(s_defaultWindow, s_ctx.m_mx, s_ctx.m_my, s_ctx.m_scroll);
					return true;
